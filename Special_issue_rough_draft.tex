...
%	\end{enumerate}
    ...
    To validate our methodology, we attempt to replicate viscosity estimates available on the NIST Reference Simulation Data website \cite{NIST_SRSW} for TraPPE-UA ethane as well as literature values for TraPPE-UA \textit{n}-octane \cite{Nieto2006}. Figures \ref{fig:validation_runs} and \ref{fig:validation_runs2} compare the ethane and \textit{n}-octane results, respectively.
    ...
    \begin{figure}[htb!]
    	...
    	\caption{Comparison with NIST Reference Simulation Data. Red line corresponds to $x = y$, i.e., exact agreement between this work and the NIST values. A constant error of 4 \% is assumed for NIST values \cite{NIST_SRSW}.}
    ...
    Figure \ref{fig:validation_runs2} demonstrates that our results agree quite well with the literature. Note that Reference \citenum{Kioupis2000} utilizes the same harmonic potential but performed NEMD simulations, while Reference \citenum{Nieto2006} performed EMD simulations with fixed bond-lengths. Thus, the comparison between literature and ``this work'' should only be made for circle symbols and square symbols.  Note that there are small discrepancies between the state points reported in Reference \citenum{Kioupis2000} and Reference \citenum{Nieto2006}. This should not impact our validation, but is somewhat curious. 
