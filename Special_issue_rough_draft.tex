...
%	\end{enumerate}
	...
	Previous studies have demonstrated that finite-size effects are typically negligible for viscosity estimates with equilibrium molecular dynamics. However, since this analysis is typically not reported in the literature, the ``Best Practices'' guide recommends validating that finite-size effects are indeed negligible. For this reason, Figure \ref{fig:finite_size_effects} compares the results for simulations performed with 100, 200, 400, and 800 molecules of BLANK and BLANK.   
	...
	This approach is employed in place of the standard Analysis of Variance (ANOVA) test because it is not possible to compute a reliable $F$-statistic since an individual replicate simulate does not provide a meaningful estimate of $\eta$. Therefore, we can only compute the sum-of-squares between averages and not the sum-of-squares between replicates.  
